...
if( Tools::isSubmit('getRank') ) {
    ...
    $data = Db::q('SELECT * FROM '._DB_PREFIX_.'points 
                  LEFT JOIN '._DB_PREFIX_.'users ON g_points.id_player = g_users.id 
    ...
    //Format data
    $html .= '<table><tr style="background:white; color:black;"><td>Nickname</td><td>Points</td><td>Map</td><td>signedin</td></tr>';
    ...
    foreach($data as $key => $value) {
        ...
        //Add row
        $html .= '<tr><td>'.$value['playernick'].'</td><td>'.$value['points'].'</td><td>'.$value['mapTitle'].'</td><td>'.$formattedDate.'</td></tr>';
    }
    $html .= '</table>';
