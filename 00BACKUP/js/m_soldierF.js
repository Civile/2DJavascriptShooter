...
function __soldierF(xpos, ypos, width, height, race, health, elid)
{
    this.eType = MOVER;
    ...
    this.height = 30;
    this.radius = 5;
    this.health = health != null ? health : 10;
    this.race = 'soldierF';
    ...
    */
    this.checkBounds = function () {
        ...
        if (__sections.getZoneByX(this.xpos + this.width) != this.zoneIndex)
            obstacles = __sections.retrieveObjs_ext(this.zoneIndex, __sections.getZoneByX(this.xpos + this.width), new Array('floor', 'wall', 'door', 'player'));
        else obstacles = __sections.retrieveObjs(this.zoneIndex, new Array('floor', 'wall', 'door', 'player'));

        if ((response = this.px.collision.satDetect(this, obstacles))) {

            if (response.obstacle === this.target && this.alert) {
                c_damage(this, this.target, this, this.direction, 1);
                c_knockback(1, Math.atan2(this.ypos - this.target.ypos, this.xpos - this.target.xpos), this.target, 1);
                this.health = 0;
                this.target.freeze = 800;
            ...
            if (response.overlapN.x) {
                ...
                this.invertVelocity();
                this.alert = false;
                if (this.xvel >= this.maxXVel && this.alert)
                    this.health = 0;
            }
    ...
    */
    this.soundEmitter = function (action) {
        ...
        switch (action) {
            ...
            case 'aggressive':
                return s_loadSoundHandler('M_SOLDIER_AGGRESSIVE2', null, "none");
            default: return;
        }
    }
