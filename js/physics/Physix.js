...
function collision()
{
    ...
    =====================
    */
    this.detect = function (entity, velocity, sensibles)
    {
        ...
        for (var i in sensibles)
            if (this.vertexIntersecate(entity, velocity, sensibles[i]))
    ...
    /*
    =====================
    Check intersecating bounds | rectangles
    =====================
    */
    this.vertexIntersecate = function (entity, velocity, boundingBox)
    {
        var collInfo = null;

        var Axl = boundingBox.xpos;
        var Axr = boundingBox.xpos + boundingBox.width;
        var Ayt = boundingBox.ypos;
        var Ayb = boundingBox.ypos + boundingBox.height;

        var Bxl = entity.xpos;
        var Bxr = entity.xpos + entity.width;
        var Byt = entity.ypos;
        var Byb = entity.ypos + entity.height;

        //ADD: Sposta fuori da qui
        if (boundingBox === entity)
            return false;

        if (Ayb < Byt || Ayt > Byb || Axr < Bxl || Axl > Bxr)
            return false;

        return this.collisionInfo = {
                objRef: boundingBox,
                momentX: Math.abs(Axl - entity.width),
    ...
    /*
    =====================
    Polygon to polygon detect
    =====================
    */
    this.satDetect = function (ent, solids, vel) {

        var B = SAT.Box;
        var V = SAT.Vector;
        var response = new SAT.Response();

        var bx1 = new B(new V(ent.xpos + (vel != null ? vel : 0), ent.ypos), ent.width, ent.height).toPolygon();
        var bx2 = null;
        ...
        for (var x in solids) {
            ...
            bx2 = new B(new V(solids[x].xpos, solids[x].ypos), solids[x].width, solids[x].height).toPolygon();
            if(SAT.testPolygonPolygon(bx1, bx2, response))
            {
                //Add a reference to the obstacle too
                response.obstacle = solids[x];
                return response;
            }
        }
        return false;
    ...
    this.satDetect_ant = function (ent, solids, distance) {

        var B = SAT.Box;
        var V = SAT.Vector;
        var response = new SAT.Response();

        var bx1 = new B(new V(ent.xpos + distance, ent.ypos), ent.width, ent.height).toPolygon();
        var bx2 = null;
    ...
    /*
    =====================
    Shit collision detect
    =====================
    */
    this.shitDetect = function (entity, solidObjs) {

        if (entity.xvel > 0) {
            for (var x in solidObjs) {
                if (solidObjs[x].xpos > entity.xpos)
                    if ((entity.xpos + entity.width + entity.xvel) >= solidObjs[x].xpos && (solidObjs[x].ypos + solidObjs[x].height) > entity.ypos && solidObjs[x].ypos < (entity.ypos + entity.height / 2)) {
                        
                        var i = new Object();
                        i.obstacle = solidObjs[x];
                        return i;
                    }
            }
        }
            ...
            for (var x in solidObjs) {
                if (solidObjs[x].xpos < entity.xpos)
                    if (entity.xpos + entity.xvel <= (solidObjs[x].xpos + solidObjs[x].width) && (solidObjs[x].ypos + solidObjs[x].height) > entity.ypos && solidObjs[x].ypos < (entity.ypos + entity.height / 2)) {
                        var i = new Object();
                        i.obstacle = solidObjs[x];
                        return i;
                    }
    ...
    /*
    =====================
    Shit Y collision detect
    =====================
    */
    this.shitYDetect = function (entity, solidObjs) {

        for (var key in solidObjs) {
            if ((solidObjs[key].xpos) <= (entity.xpos + (entity.width / 2)) && (solidObjs[key].xpos + solidObjs[key].width) > (entity.xpos + (entity.width / 3))) {
                if (entity.yvel > 0) {
                    if ((entity.ypos + entity.height + entity.yvel) >= solidObjs[key].ypos && solidObjs[key].ypos > entity.ypos) //down collision
                    {
                        var i = new Object();
                        i.obstacle = solidObjs[key];
                        return i;
                    }
                }
                else if (entity.yvel < 0) {
                    if (entity.ypos + entity.yvel <= (solidObjs[key].ypos + solidObjs[key].height) && solidObjs[key].ypos < entity.ypos) //up collision
                    {
                        var i = new Object();
                        i.obstacle = solidObjs[key];
                        return i;
    ...
    this.rectXLTPoint = function (rect) {
        return rect.xpos;
    }

    this.rectXRTPoint = function (rect) {
        return rect.xpos + rect.width;
    }

    this.rectYLTPoint = function (rect) {
        return rect.ypos
    }

    this.rectYLBPoint = function (rect) {
        return rect.ypos + rect.height;
    }
