...
function g_start() {
    s_preloadSounds(new Array(a_sounds, m_music, p_sounds), g_firstStart, function () {
        $('#loadOutput').html('<div style="background:black; height:30px; color:white; padding:5px; font-size:10px; text-align:right; width:'+(s_getPercentage() - 3)+'%;">'+s_getPercentage()+'%&nbsp;</div>');
    });
}
...
function startSkyAnimations(first, clear) {

    if (typeof clear === "undefined")
        clear = true;

    $('#game').stop(true, clear).animate({
        backgroundColor: first || 'rgba(115, 153, 57, 0.61)'
    }, {
        duration: 30000,
        complete: function () {
            $('#game').animate({
                backgroundColor: 'rgba(115, 153, 57, 0.61)'
            }, {
                duration: 30000,
                complete: function () {
                    $('#game').animate({
                        backgroundColor: 'rgba(112, 2, 8, 0.611765);'
                    }, {
                        duration: 30000,
                        complete: function () {
                            startSkyAnimations('rgba(9, 34, 66, 0.61)', false);
                        }
...
function gameLoop() {
    ...
    if (game.playing && !game.ended) {
        ...
        game.deltaT = (current - game.lastTime) / 1000;
...
==============================================================================================================================
*/
var S_MAXENTITIES = 8;   //ADD: ricava width mappa e calcola max (es. width 7000 -> 7 + 3)
var S_NUMENTITIES = 0;
var gs_playing = true;
var s_spawnTime = 1000;
var monsters = new Array('soldier', 'soldierF', 'soldierC');
...
function g_processGameTypeEvents() {
    ...
    switch (monsters[randomMonster]) {
        case 'soldier':
            monster = new __soldier(expos, 15, null, null, 'soldier', 50, 1);
            break;
        case 'soldierF':
            monster = new __soldierF(expos, 15, null, null, 'soldierF', 50, 1);
            break;
        case 'soldierC':
            monster = new __soldierC(expos, 15, null, null, 'soldierA', 50, 1);
            break;
        case 'heavySoldier':
            monster = new __heavySoldier(expos, 15, null, null, 'heavySoldier', 50, 1);
            break;
        case 'giantSoldier':
            monster = new __giantSoldier(expos, 15, null, null, null, 50, 1);
            break;
    }
...
function gs_spawnItems() {    
    ...
    if (itemType == 'w_') {
        ...
        if (!w_areAmmoUseful(itemType)) return;
        interactions = { interaction: 'ammoBox' }
    }
    else
    {
        if (randomItem == 'healthBox' && player.health >= 100)
            return;
...
//Save points
function gs_savePoints(points, name, onstart, onsuccess) {
    ...
    $.ajax({
        type: 'POST',
        data: 'savePoints=1&points=' + points + '&mapTitle=S1&name=' + name || "unnamed",
...
function gs_showResultBox(points) {
    ...
    $(WINDOW_WND).append('<div id="resultBox" style=""><div class="resultBox points"></div><div class="resultBox restart"><input type="button" value="RESTART" onclick="javascript:restart()" style="border:0; background:#ffffff"></div><div>Your name: <input type="text" name="player-name" class="player-name" value="" /><input type="button" class="save-p" name="save-p" value="CONFIRM" /></div></div>');

    $(".save-p").focus().off().on("click", function () {
        game.inputstate = false;
        gs_savePoints(points, $(".player-name").val(), function () {
            $(".save-p").val("saving...");
        }, function () {
            $(".save-p").val("SAVED!");
        });
